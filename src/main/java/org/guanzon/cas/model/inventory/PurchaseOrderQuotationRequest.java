 ...
 * @author Maynard
 */
public class PurchaseOrderQuotationRequest implements GTranDet {
    ...
    public PurchaseOrderQuotationRequest(GRider foGRider, boolean fbWthParent) {
        poGRider = foGRider;
    ...
    public JSONObject OpenModelDetail(String fsTransNo) {
        ...
        try {
            ...
            while (loRS.next()) {
                ...
                poModelDetail.get(poModelDetail.size() - 1).openRecord(loRS.getString("sTransNox"), loRS.getString("sStockIDx")); //given parameter is 1
                if ("error".equals((String) poJSON.get("result"))) {
                    return poJSON;
                } else {
                    poJSON = new JSONObject();
